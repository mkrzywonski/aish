...
func (c *Core) fileRead(ctx context.Context, req *mcp.CallToolRequest, args fileReadArgs) (*mcp.CallToolResult, fileReadResult, error) {
	...
	case "sftp":
		if o.lines {
			return nil, fileReadResult{}, errors.New("SFTP supports byte reads only; use offset/max_bytes instead of start_line/limit")
		}
		read, readErr := c.Mux.SFTPRead(ctx, rt.ci, args.Path, o.offset, o.max+utf8.UTFMax)
		err = readErr
		sample = fileReadSample{data: read.Data, offset: o.offset, eof: read.EOF}
	case "in_band":
