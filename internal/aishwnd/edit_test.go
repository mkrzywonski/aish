...
func TestFileEditRoundTrip(t *testing.T) {
	...
	sess := newMultiTestWirePair(t, "test-fe", func(t *testing.T, peer *aishwinwire.Conn, f aishwinwire.Frame) {
		switch f.Type {
		case "file_read":
	...
	wantVersion := aishwinwire.SHA256Version([]byte(original))
...
func TestFileEditRejectsAmbiguousMatch(t *testing.T) {
	sess := newMultiTestWirePair(t, "test-fe2", func(t *testing.T, peer *aishwinwire.Conn, f aishwinwire.Frame) {
		if f.Type != "file_read" {
...
func TestFilePatchRoundTrip(t *testing.T) {
	...
	sess := newMultiTestWirePair(t, "test-fp", func(t *testing.T, peer *aishwinwire.Conn, f aishwinwire.Frame) {
		switch f.Type {
		case "file_read":
			data, _ := json.Marshal(aishwinwire.FileReadResultData{
				Content: base64.StdEncoding.EncodeToString([]byte(original)),
...
func TestFilePatchMismatchNeverWrites(t *testing.T) {
	sess := newMultiTestWirePair(t, "test-fp2", func(t *testing.T, peer *aishwinwire.Conn, f aishwinwire.Frame) {
		if f.Type != "file_read" {
			t.Errorf("unexpected frame type %q (should have failed before file_write)", f.Type)
			return
		}
