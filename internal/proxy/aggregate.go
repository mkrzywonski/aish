...
const serverInstructions = "Aish gives you access to human-owned shared terminal sessions and to the current host " +
	...
	"explicit, and may trigger MFA; never use it instead of oob_tools. Explicit SFTP may trigger MFA; only a sticky " +
	"shell failure permits read-only SFTP file routing. Every " +
