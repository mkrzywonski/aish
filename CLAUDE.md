...
- `internal/proxy` (`aish mcp-proxy`, `aggregate.go`) is a durable, session-aware MCP SERVER — its lifetime is the AI TUI's, independent of any session. It presents one endpoint over stdio, mirrors the session tool set (fetched from a live session's `ListTools`, cached to `~/.cache/aish/tools.json` so it works with zero sessions; `authorize` filtered out) plus a local `list_sessions`, and routes each `tools/call` to the session named in its `session` arg. It holds ONE connection per session in a pool (`conn`), opened lazily and NOT token-authorized — so the session's own y/n approval fires once per session per TUI lifetime, on that session's terminal (the positive target ID). Reuse = no re-prompt; session/TUI close drops the connection and clears approval. `resolve` errors on ambiguity (several sessions, no `session` arg) — never guesses. `Discover`/`List`/`Resolve` in `proxy.go` do session enumeration.
...
- Console (`session/console.go`): the sanctioned SECOND exception to byte-transparency (window-title marking was the first). `Notify`/`Prompt` write to the real terminal (os.Stdout) and, for a prompt, capture the user's keystroke off the stdin pump before it reaches the shell — so aish can ask the user for consent without a byte entering the session stream. `outMu` gates the output pump while a prompt owns the screen (shell output waits, flushes after, never interleaves); `promptMu` serializes prompts; `capturing`/`capCh` divert stdin.
- Connection authorization (`mcpserver/connauth.go`): a new MCP connection can't call any tool until the user approves it with a y/n prompt in the terminal (`Sess.Prompt`, shown on first gated tools/call). This is a consent/awareness control, NOT a defense against same-uid code — the socket is 0700 so only same-uid can connect, and a hostile same-uid process is explicitly out of scope (browser JS can't reach a unix socket at all, which is the real reason not to ever expose MCP over TCP). y approves for the connection; n is a sticky deny (reconnect to retry); timeout fails closed. Internal same-uid helpers (cross-session forwarding, debug CLI) present the per-session token (`paths.TokenFile`, 0600) via the `authorize` tool and are never prompted. `--no-auth` (`Core.NoAuth`) disables the prompt. `connAuthMiddleware` is registered OUTERMOST (before cross-session); state keyed by `*mcp.ServerSession`, cleared on disconnect.
...
Runtime state lives in `$XDG_RUNTIME_DIR/aish/<session-id>/` (`internal/paths`): MCP socket, `name`, `token` (connection-auth bypass for internal clients), `oob` (out-of-band grant marker, read by the shim), `bin/ssh` shim symlink, `cm-*` ControlMaster sockets, `ssh/*.json` connection events. The dir is removed on exit/signal; ControlPersist=60s reaps orphaned masters after SIGKILL.
