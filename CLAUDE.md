...
- Connection authorization (`mcpserver/connauth.go`): a new MCP connection can't call any tool but `authorize` until it proves the operator can see this terminal. On the first gated call, a per-connection 6-digit code is shown via `Notify`; the human reads it to the AI, which submits it through the `authorize` tool. Internal same-uid clients (cross-session forwarding, debug CLI) present the per-session token (`paths.TokenFile`, 0600) instead, so they never spam the terminal. `connAuthMiddleware` is registered OUTERMOST (before cross-session). State is keyed by `*mcp.ServerSession`, cleared on disconnect.
