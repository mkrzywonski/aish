...
- `internal/framing.Engine` implements `run_command` with three strategies chosen at call time: OSC 133 C→D window when `Tracker.PromptReady()` (integration active), else invisible OSC-7979 nonce sentinel appended to the command (non-integrated shells, remotes), idle timeout as last resort. The echoed command never triggers the parser because the typed `\033` is literal text until the remote printf executes.
